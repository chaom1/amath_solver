...
The solver preserves the identity and point value of every physical tile, including blank and flexible-operation tiles. It validates equation structure, supports standard operator precedence and chained equalities, and uses exact fractional arithmetic so its results do not depend on rounding. Scores include the value of all tiles in an equation, selectable double/triple tile and equation squares, and a 40-point bonus when eight tiles from the hand are used. Special squares apply only to newly placed tiles.

The project supports board lines from 3 to 15 cells, hands of up to 15 tiles, and displays up to 30 ranked results. It can analyze an empty-board first move or build an equation through locked tiles already on the selected line. To keep large searches responsive, the solver uses an analysis limit and clearly marks results as partial when that limit is reached.
